.PHONY: clean build deploy run frontend backend stop_backend check_ports

# 设置默认目标不是文件
.PHONY: clean build deploy run frontend backend

# 运行应用，用于开发和测试
run:
ifeq ($(filter frontend,$(MAKECMDGOALS)),frontend)
	@$(MAKE) run_frontend
else ifeq ($(filter backend,$(MAKECMDGOALS)),backend)
	@$(MAKE) run_backend
else
	@echo "Please specify 'frontend' or 'backend' as an argument to 'make run'."
endif
