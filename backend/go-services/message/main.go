...
import (
	"context"
	"github.com/BigNoseCattyHome/aorb/backend/go-services/message/services"
...
func main() {
	...
	//profiling.InitPyroscope("AorB.MessageService")

	log := logging.LogService(config.MessageRpcServerName)
	lis, err := net.Listen("tcp", config.Conf.Pod.PodIp+config.MessageRpcServerAddr)
	...
	if err := consul.RegisterConsul(config.MessageRpcServerName, config.MessageRpcServerAddr); err != nil {
		log.Panicf("Rpc %s register consul happens error for: %v", config.MessageRpcServerName, err)
	}
	log.Infof("Rpc %s is running at %s now", config.MessageRpcServerName, config.MessageRpcServerAddr)

	var srv services.MessageServiceImpl
	...
	httpSrv := &http.Server{Addr: config.Conf.Pod.PodIp + config.MessageMetrics}
	g.Add(func() error {
		m := http.NewServeMux()
