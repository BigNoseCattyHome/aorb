...
import (
	...
	"github.com/BigNoseCattyHome/aorb/backend/services/poll/models"
)
...
func (s PollServiceImpl) CreatePoll(ctx context.Context, request *pollPb.CreatePollRequest) (resp *pollPb.CreatePollResponse, err error) {
	...
	filter4InsertPollUuid2PollAsk := bson.D{
		{Key: "username", Value: newPoll.UserName},
	}
	update4InsertPollUuid2PollAsk := bson.D{
		{Key: "$push", Value: bson.D{
			{Key: "pollask.pollids", Value: newPoll.PollUuid},
		}},
	}
...
func (s PollServiceImpl) FeedPoll(ctx context.Context, request *pollPb.FeedPollRequest) (response *pollPb.FeedPollResponse, err error) {
	...
	filter := bson.D{
		{Key: "createAt", Value: bson.D{{Key: "$lt", Value: latestTime}}},
		{Key: "pollUuid", Value: bson.D{{Key: "$nin", Value: redisKeys}}},
	}
	options := options.Find().SetSort(bson.D{{Key: "createAt", Value: 1}}).SetLimit(int64(10 - len(rPollList)))
...
func BuildPollPbModel(poll *models.Poll) *pollPb.Poll {
