...
func ScanGet(ctx context.Context, key string, obj interface{}, collectionName string) (bool, error) {
	...
	if err := redis.RedisClient.HGetAll(ctx, key).Scan(obj); err != nil {
		if err != redis2.Nil {
			logger.WithFields(logrus.Fields{
				"key": key,
				"err": err,
	...
	if result := redis.RedisClient.HSet(ctx, key, obj); result.Err() != nil {
		logger.WithFields(logrus.Fields{
			"err": result.Err(),
			"key": key,
...
func ScanTagDelete(ctx context.Context, key string, obj interface{}) {
	...
	redis.RedisClient.HDel(ctx, key)
...
func Get(ctx context.Context, key string) (string, bool, error) {
	...
	if result = redis.RedisClient.Get(ctx, key); result.Err() != nil && result.Err() != redis2.Nil {
		logger.WithFields(logrus.Fields{
			"err":    result.Err(),
			"string": key,
...
func Write(ctx context.Context, key string, value string, state bool) {
	...
	if state {
		redis.RedisClient.Set(ctx, key, value, 120*time.Hour+time.Duration(rand.Intn(redisRandomScope))*time.Second)
	}
}
